...
program
  .command("analyze")
  .description(
    "Fleet cost drivers from the session index, or a per-session inefficiency report with --id.",
  )
  ...
  .action(async (o) => {
    ...
    if (!idx) return;
    const pct = (r?: number) =>
      r === undefined ? "—" : `${Math.round(r * 100)}%`;
    if (o.id) {
      const rec = idx.sessions.find((s) => s.id === o.id);
      if (!rec)
        throw new Error(
          "analyze --id did not match any indexed session. Run: vfr sessions",
        );
      ...
      for (const m of r.cost.perModel)
        console.log(
          `${shortModelName(m.model).padEnd(17)} ${formatTokenCount(m.tokens.inputTokens).padEnd(8)} ${formatTokenCount(m.tokens.outputTokens).padEnd(8)} ${formatTokenCount(m.tokens.cacheCreationTokens).padEnd(8)} ${formatTokenCount(m.tokens.cacheReadTokens).padEnd(8)} ${formatUsd(m.usd)}`,
        );
      ...
      console.log(
        `Context curve: ${r.contextCurve.turns} turns · first ${formatTokenCount(r.contextCurve.firstTokens)} · peak ${formatTokenCount(r.contextCurve.peakTokens)} · final ${formatTokenCount(r.contextCurve.finalTokens)}`,
      );
      if (r.topContributions.length) {
        ...
        for (const c of r.topContributions)
          console.log(
            `- call ${c.callIndex + 1}/${r.totalCalls} (${c.label.replace(/\s+/g, " ").slice(0, 60)}): added ${formatTokenCount(c.addedTokens)} tokens, carried ${c.callsCarried} calls ≈ ${c.lifetimeUsd === undefined ? "unpriced" : formatUsd(c.lifetimeUsd)}`,
          );
        const covered =
          r.contextUsd > 0 ? Math.round((r.contributionsUsd / r.contextUsd) * 100) : 0;
        console.log(
          `All ${r.totalCalls} contributions sum to ${formatUsd(r.contributionsUsd)} vs actual context cost ${formatUsd(r.contextUsd)} (${covered}% reconciled); output ${formatUsd(r.outputUsd)} → session total ${formatUsd(r.cost.totalUsd)}`,
        );
      }
      if (r.topToolOutputs.length) {
        ...
        for (const t of r.topToolOutputs)
          console.log(
            `- #${t.eventIndex} ${t.type} ${t.title.replace(/\s+/g, " ").slice(0, 60)} — ${t.chars} chars (~${formatTokenCount(t.approxTokens)} tokens)`,
          );
      }
      if (r.flags.length) {
        console.log("\nFlags:");
    ...
    console.log(
      `Sessions: ${r.totals.sessions} (${r.totals.pricedSessions} priced, ${r.totals.unpricedSessions} unpriced)`,
    );
    console.log(`Total estimated cost: ${formatUsd(r.totals.totalCostUsd)}`);
    console.log(
      `Tokens: input ${formatTokenCount(r.totals.tokens.input)} · output ${formatTokenCount(r.totals.tokens.output)} · cache write ${formatTokenCount(r.totals.tokens.cacheCreation)} · cache read ${formatTokenCount(r.totals.tokens.cacheRead)}`,
    );
    if (r.costByClass)
      console.log(
        `Cost by class (approx, ${r.costByClass.coveredSessions} sessions): input ${formatUsd(r.costByClass.inputUsd)} · output ${formatUsd(r.costByClass.outputUsd)} · cache write ${formatUsd(r.costByClass.cacheWriteUsd)} · cache read ${formatUsd(r.costByClass.cacheReadUsd)}`,
      );
    console.log("\nTop sessions by cost");
    console.log(
      "ID                 Project              Cost     CacheHit  Tokens   Model",
    );
    for (const s of r.topSessions)
      console.log(
        `${s.id.padEnd(18)} ${s.project.slice(0, 20).padEnd(20)} ${(s.unpriced ? "—" : formatUsd(s.costUsd ?? 0)).padEnd(8)} ${pct(s.cacheHitRatio).padEnd(9)} ${formatTokenCount(s.tokenCount).padEnd(8)} ${s.model ? shortModelName(s.model) : "-"}`,
      );
    console.log("\nTop projects by cost");
    console.log("Project              Sessions  Cost     Tokens");
    for (const p of r.topProjects)
      console.log(
        `${p.project.slice(0, 20).padEnd(20)} ${String(p.sessions).padEnd(9)} ${formatUsd(p.costUsd).padEnd(8)} ${formatTokenCount(p.tokenCount)}`,
      );
    console.log(
      `\nTop ${r.concentration.topN} sessions carry ${pct(r.concentration.shareOfTotal)} of fleet cost.`,
    );
    console.log(
      `Subagents: ${r.subagents.sessionCount} sessions, ${formatUsd(r.subagents.costUsd)} (${pct(r.subagents.shareOfTotal)} of fleet cost).`,
    );
    if (r.flags.length) {
      console.log("\nFlags:");
      for (const f of r.flags) console.log(`- ${f.message}`);
    ...
    if (r.totals.unpricedSessions > 0)
      console.log(
        `${r.totals.unpricedSessions} sessions (codex/pi/unknown) unpriced: ${formatTokenCount(r.totals.unpricedTokenCount)} tokens.`,
      );
  });
program
  .command("browse")
