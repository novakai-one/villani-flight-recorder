...
function recordCacheHitRatio(s: SessionRecord): number | undefined {
  if (
...
function classUsd(s: SessionRecord) {
  ...
  return {
    ...
    cacheWriteUsd:
      ((s.cacheCreationTokenCount ?? 0) * m.rates.cacheWrite) / 1_000_000,
    cacheReadUsd:
      ((s.cacheReadTokenCount ?? 0) * m.rates.cacheRead) / 1_000_000,
  };
}

const projectOf = (s: SessionRecord) =>
  s.projectDisplayName ?? s.projectName ?? "-";
...
export function analyzeFleet(
  ...
  for (const s of sessions) {
    ...
    if (c) {
      ...
      const sessionClassUsd =
        c.inputUsd + c.outputUsd + c.cacheWriteUsd + c.cacheReadUsd;
  ...
  const projects = new Map<
    string,
    { project: string; sessions: number; costUsd: number; tokenCount: number }
  >();
  for (const s of sessions) {
...
export type ContextContribution = {
  /** Index of the API call (usage-bearing event) that ingested the tokens. */
  callIndex: number;
  eventIndex: number;
  /** What most likely added the tokens: largest tool output since the prior call. */
  label: string;
  /** Exact new-context tokens at this call: input + cacheCreation. */
  addedTokens: number;
  /** Subsequent API calls that re-read these tokens from cache. */
  callsCarried: number;
  ingestUsd?: number;
  carryUsd?: number;
  /** ingest + carry: what these tokens cost over the life of the session. */
  lifetimeUsd?: number;
};
...
function contextContributions(
  ...
  for (const c of out) {
    ...
    const rates = matchModelRates(
      events[c.eventIndex].model ?? rec.model ?? "",
    )?.rates;
    if (rates) {
      c.carryUsd =
  ...
  for (const c of out) {
    if (c.carryUsd === undefined) continue;
    c.carryUsd *= scale;
...
export function analyzeSession(rec: SessionRecord, events: FlightEvent[]) {
  const cost: CostEstimate = estimateCost(events);
  let input = 0,
    cacheCreation = 0,
    cacheRead = 0;
  ...
  const topContributions = [...contributions]
    .sort(
      (a, b) =>
        (b.lifetimeUsd ?? -1) - (a.lifetimeUsd ?? -1) ||
  ...
  const contributionsUsd = contributions.reduce(
    (a, c) => a + (c.lifetimeUsd ?? 0),
    0,
  );
